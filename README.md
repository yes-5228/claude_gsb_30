...
| GET/PATCH/DELETE | `/inspections/{id}` | 详情 / 更新（改分联动已登记问题，响应带 `issue_sync` 摘要）/ 删除 |
...
- **评价变更联动问题**：巡查改分或删减检查项时，同一事务内重判该巡查登记出的「待整改」问题（已进入整改流程的一律保留）：来源检查项被删除或改分后达标的，问题自动「作废关闭」；仍不达标且分数变化的，按新分数重算严重程度（<3 紧急、<5 严重、其余一般）并校准分类；判断未变或手工上报的问题保持不动。每次联动都会写入整改流水，改分与问题联动要么一起成功、要么整体回滚，看板与台账的问题数因此始终一致。
...
- 后端 `pytest`：10 个用例全部通过；`/health`、台账/巡查/问题/统计/字典接口均返回预期数据。
