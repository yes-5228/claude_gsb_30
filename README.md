...
| GET/PATCH/DELETE | `/inspections/{id}` | 详情 / 更新 / 删除；PATCH 与 DELETE 支持 `issue_mode` 控制关联问题如何联动 |
...
- **评价与问题联动**：由巡查检查项登记的问题（`source_item` 非空）随巡查评价联动管理。对巡查改分时默认 `issue_mode=adjust`，按检查项一次性对账：仍不达标的项保留问题并按新分数调整分类与严重程度、已达标或被删除项的问题作废关闭、新增不达标项自动登记问题；`keep` 保留原问题不动，`void` 整批作废。删除巡查时默认 `void`（作废并保留整改轨迹），也可 `unlink`（解除关联、问题保留）或 `delete`（一并物理删除）。作废问题保留历史痕迹但状态置为「已关闭」并移出看板/台账统计，因此看板问题总数与台账详情始终对得上；手工上报、群众反馈等非巡查登记的问题不受影响。巡查写入与问题联动在同一事务内一次提交，中途失败整体回滚，不会留下改了一半的问题集合。
...
- 后端 `pytest`：19 个用例全部通过；`/health`、台账/巡查/问题/统计/字典接口均返回预期数据。
